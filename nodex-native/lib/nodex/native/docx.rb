...
# DOCX/ODT export for Nodex nodes — native C++ rendering.
...
module Nodex
  ...
  module Native
    ...
    #   docx = Nodex::Native.to_docx(node)
    #   docx = Nodex::Native.to_docx(node, "page_size" => "A4")
    def self.to_docx(node, opts = nil)
      NativeDocx.render_docx(node, opts)
    end

    # Render a node tree to ODT format (binary string).
    #   odt = Nodex::Native.to_odt(node)
    #   odt = Nodex::Native.to_odt(node, "page_size" => "A4")
