...
[← Главная](index.md) · [nodex-native](nodex-native.md) · [baked-templates](baked-templates.md) · [packed-builder](packed-builder.md)
...
```bash
cd nodex-native && rake compile && ruby test/bench_levels.rb
```
...
### Worker scaling (Puma-style, 12 ядер)

```bash
ruby test/bench_workers.rb
```

| Режим | 1 worker | 2 workers | 4 workers | 8 workers |
|-------|----------|-----------|-----------|-----------|
| to_html (5-node) | 30k ops/s | 1.9x | 2.6x | **4.2x** |
| render_baked (3 slots) | 36k ops/s | 1.7x | 2.9x | **5.0x** |
| PackedBuilder (5-node) | 32k ops/s | 1.7x | 2.6x | **3.7x** |
| to_html (50-card page) | 2.8k ops/s | 1.9x | 2.0x | **3.5x** |

Масштабирование через worker processes (как Puma `workers`). Каждый worker — отдельный процесс со своим GVL, реальный параллелизм на всех ядрах.

> **Примечание:** Ruby threads ограничены GVL и не дают прироста на CPU-bound рендеринге. Для масштабирования используйте Puma workers (fork), не threads.
