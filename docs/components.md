...
`FWUI::Registry` — центральный реестр компонентов и страниц. Компоненты — переиспользуемые блоки UI. Страницы — маршруты, которые возвращают полный HTML-документ.
...
page = registry.create_page("/project", { id: "fwui" })
```

## PageLoader — автозагрузка

`FWUI::PageLoader` автоматически загружает все `.rb` файлы из директории и вызывает `register(registry)`:

```ruby
loaded = FWUI::PageLoader.load_pages(registry, "ruby/pages/")
puts loaded  # => ["home"]
```
...
- `extend FWUI::DSL` даёт доступ к h1(), div(), Bold() без префикса
