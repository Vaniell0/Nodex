...
FWUI::Native.bake(:card, FWUI.div([
  FWUI.h1(FWUI.slot(:title)).bold.color("#333"),
  FWUI.p(FWUI.slot(:desc)).padding("10px"),
  FWUI.a("Link", href: FWUI.slot(:link)),
]).add_class("card"))
```

`FWUI.slot(:name)` — маркер слота. При рендере Node-дерева он превращается в `__FWUI_SLOT_name__`, затем `bake()` разрезает HTML по маркерам.

### Рендер

```ruby
FWUI::Native.render_baked(:card,
  title: "Project X",
  desc: "A cool project",
  link: "/projects/x"
)
...
```ruby
page = FWUI.div([
  FWUI::Native.baked_node(:card, title: "A", desc: "First", link: "/a"),
  FWUI::Native.baked_node(:card, title: "B", desc: "Second", link: "/b"),
])
puts page.to_html
```
...
`baked_node` рендерит шаблон и оборачивает в `FWUI.raw()`.

### Простой шаблон

```ruby
FWUI::Native.bake(:greeting, FWUI.div([
  FWUI.h1(FWUI.slot(:title)).bold,
  FWUI.p(FWUI.slot(:body)),
]).add_class("card"))

FWUI::Native.render_baked(:greeting, title: "Hello", body: "World")
```
...
def self.bake(name, template_node)
  ...
  parts = html.split(/__FWUI_SLOT_(\w+)__/, -1)
